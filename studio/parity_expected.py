...
Fixtures are business/sample/*.csv, which are themselves generated by a seeded
script — so this whole chain is reproducible.
"""
...
SAMPLE = os.path.join(ROOT, "business", "sample")
...
def main():
    if not os.path.isdir(SAMPLE):
        print("studio/parity_expected: business/sample missing — run "
              "python3 business/sample.py first", file=sys.stderr)
        return 2

    out = {}

    # --- concentration + vintages on the page fixture --------------------
    items, dated = load_items(os.path.join(SAMPLE, "pages.csv"),
    ...
    ritems, _ = load_items(os.path.join(SAMPLE, "revenue.csv"), "source", "amount")
    ...
    # --- trend detection --------------------------------------------------
    rows = csvio.read_rows(os.path.join(SAMPLE, "traffic.csv"), quiet=True)
    pts = []
    for r in rows:
