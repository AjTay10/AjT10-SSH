...
description: >
  ...
  Mastodon, Telegram, Pinterest, YouTube, Wikipedia, Hacker News or Stack
  Overflow — and for reading or searching any ordinary web page.
...
Check availability first if unsure: `reach doctor` probes all 17 platforms live
in about 30 seconds and prints which backend currently serves each one.
...
2. Some platforms only yield metadata or a search index because they gate post
   ...
   Pinterest, Reddit. `reach` prints a `[note: ...]` banner when it is handing
